...
#2a:
#changing to uppercase to homogenize it
...
demo_train$Child.ID <- str_replace_all(demo_train$Child.ID,"[[:punct:]]","")
...
new <- merge(demo_train_sel,LU_train_sel,by=c("SUBJ","VISIT"))
merged<- merge(new,token_train_sel,by=c("SUBJ","VISIT"))
...
merged$SUBJ <- as.numeric(as.factor(merged$SUBJ))
