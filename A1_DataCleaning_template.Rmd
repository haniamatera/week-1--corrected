...
demo_train$Child.ID <- toupper(demo_train$Child.ID)
demo_train$Ethnicity <- toupper(demo_train$Ethnicity)

LU_train$SUBJ <- toupper(LU_train$SUBJ)

token_train$SUBJ <- toupper(token_train$SUBJ)
...
```{r}
#extracting just a number from a "visit" column 
...
#names <- demo_train$Child.ID
#demo_train$Child.ID <- str_replace_all(names,"."," ")
...
```{r}
#leaving out designated columns 
demo_train_sel <- select(demo_train, c(Child.ID,Visit,Diagnosis,Ethnicity,Gender,Age,ADOS,MullenRaw,ExpressiveLangRaw,Socialization))

LU_train_sel <- select(LU_train,c(SUBJ,VISIT,MOT_MLU,CHI_MLU))

token_train_sel <- select(token_train,c(SUBJ,VISIT,types_MOT,types_CHI,tokens_MOT,tokens_CHI))
```
...
```{r}
#renaming columns in demo_train_sel

names(demo_train_sel)[1] <- "SUBJ"
names(demo_train_sel)[2] <- "VISIT"

#merging data frames by subject

new <- merge(demo_train_sel,LU_train_sel,by="SUBJ")

merged<- merge(new,token_train_sel,by="SUBJ")

#why do I get visit.x and visit.y ???? +all the NAs? (what shall i do with guys who only had 1 visit ? like A.A etc.)+ Diagnosis fucked up ~now there are only B's
...
```{r}
#? how to fix the visit thingy 

#visit1 <- subset(merged,select=VISIT==1)
#visit1 <- select(merged,c(SUBJ,VISIT,ADOS,MullenRaw,ExpressiveLangRaw))


#merge(merged,visit1,by="SUBJ")
...
merged$Gender <- ifelse(merged$Gender==1,"F","M")

#DIAGNOSIS~ sth is wrong here

merged$Diagnosis <- ifelse(merged$Diagnosis=="A","ASD","TD")
